...
export const getAdminDashboard = async (req: Request, res: Response) => {
  try {
    ...
    // 3. Raw Tasks List for Today & Overdue Breakdowns
    const listTasksRaw: any[] = await db.sequelize.query(
      `SELECT t.id AS task_id, t.assigned_agent_id AS agent_id, su.name AS agent_name, t.lead_id, l.full_name AS lead_name, l.phone AS lead_phone, t.subject, t.location, t.task_type, t.status, t.start_at, t.end_at 
       FROM public.lead_tasks t 
       JOIN public.leads l ON l.id = t.lead_id AND l.deleted_at IS NULL 
