    ...
    public getAdminDashboard = async (req: Request, res: Response) => {
        try {
            ...
            const listTasksRaw: any[] = await this.db_services.sequelizeWriter.query(
                `SELECT t.id AS task_id, t.assigned_agent_id AS agent_id, su.name AS agent_name, t.lead_id, l.full_name AS lead_name, l.phone AS lead_phone, t.subject, t.location, t.task_type, t.status, t.start_at, t.end_at FROM public.lead_tasks t JOIN public.leads l ON l.id = t.lead_id AND l.deleted_at IS NULL JOIN public.system_users su ON su.id = t.assigned_agent_id WHERE t.deleted_at IS NULL AND su.deleted_at IS NULL AND (su.is_blocked = FALSE OR su.is_blocked IS NULL) ORDER BY t.start_at ASC`,
            ...
            listTasksRaw.forEach(t => {
                ...
                const isOverdue = isPending && dueTimeCA < nowCA;
                const isToday = startCA >= todayStartCA && startCA <= todayEndCA;
                ...
                } else if (isPending && isToday && !isOverdue) {
                    if (!todayTasksByAgent[agentKey]) todayTasksByAgent[agentKey] = { agent_id: t.agent_id, agent_name: t.agent_name, tasks: [] };
                    todayTasksByAgent[agentKey].tasks.push({
                        task_id: t.task_id,
                        lead_id: t.lead_id,
                ...
                if (isOverdue) {
                    ...
                    overdueTasksByAgent[agentKey].tasks.push({
                        ...
                        due_date: toCADate(t.end_at ?? t.start_at),
                        start_at_ca: toCAString(t.start_at),
                        end_at_ca: toCAString(t.end_at ?? t.start_at),
                    });
                }
            });
