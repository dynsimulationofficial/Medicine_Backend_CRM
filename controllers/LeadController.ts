...
export default class LeadController extends BaseController {
    ...
    public exportMultipleLeadsData = async (req: Request, res: Response) => {
        try {
            const authUser = (req as any)?.user;
            if (!authUser?.system_user_id) {
                return this.sendError(res, {}, "Unauthorized - Please login again", 401);
            ...
            const schema = Yup.object({
                ...
                created_from: s().optional(),
                created_to: s().optional(),
                lead_score: Yup.number().optional(),
                lead_quality: s().max(50).optional(),
                best_time_to_call: s().max(50).optional(),
                status: s().max(100).optional(),
                company: s().max(255).optional(),
                activity_summary: s().optional(),
                note: s().optional(),
            });
            ...
            if (qp.q) { where.push(`(l.full_name ILIKE :q OR l.email ILIKE :q OR l.phone ILIKE :q OR l.lead_number::text ILIKE :q)`); repl.q = `%${qp.q}%`; }
            if (qp.full_name) { where.push(`l.full_name ILIKE :full_name`); repl.full_name = `%${qp.full_name}%`; }
            if (qp.email) { where.push(`l.email ILIKE :email`); repl.email = `%${qp.email}%`; }
            if (qp.phone) { where.push(`l.phone ILIKE :phone`); repl.phone = `%${qp.phone}%`; }
            if (qp.lead_number) { where.push(`l.lead_number::text ILIKE :lead_number`); repl.lead_number = `%${qp.lead_number}%`; }
            if (qp.city) { where.push(`l.city ILIKE :city`); repl.city = `%${qp.city}%`; }
            if (qp.state) { where.push(`l.state ILIKE :state`); repl.state = `%${qp.state}%`; }
            if (qp.agent_ids?.length) { where.push(`l.agent_id = ANY(ARRAY[:agent_ids]::uuid[])`); repl.agent_ids = qp.agent_ids; }
            if (qp.lead_source_id) { where.push(`l.lead_source_id = :lead_source_id`); repl.lead_source_id = qp.lead_source_id; }
            if (qp.debt_consolidation_status_id) { where.push(`l.debt_consolidation_status_id = :debt_consolidation_status_id`); repl.debt_consolidation_status_id = qp.debt_consolidation_status_id; }
            if (qp.consolidated_credit_status_id) { where.push(`l.consolidated_credit_status_id = :consolidated_credit_status_id`); repl.consolidated_credit_status_id = qp.consolidated_credit_status_id; }
    
            // Date filters (from date)
            if (qp.created_from) {
                const f = parseInCA(qp.created_from);
                if (!f || !f.isValid) return this.sendError(res, {}, "Invalid created_from date", 400);
                where.push(`l.created_at >= :created_from`);
                repl.created_from = f.toUTC().toISO({ suppressMilliseconds: true });
            }
            if (qp.created_to) {
                const t0 = parseInCA(qp.created_to);
                if (!t0 || !t0.isValid) return this.sendError(res, {}, "Invalid created_to date", 400);
                const end = /^\d{2}-\d{2}-\d{4}$/.test(qp.created_to) ? t0.plus({ days: 1 }).startOf("day") : t0;
                where.push(`l.created_at < :created_to_plus`);
                repl.created_to_plus = end.toUTC().toISO({ suppressMilliseconds: true });
            }
            ...
            const rows: any[] = await this.db_services.sequelizeWriter.query(
                `
                SELECT
                    l.id, l.lead_number, l.full_name, l.email, l.phone,
                    l.address_line1, l.address_line2, l.city, l.state, l.postal_code, l.country,
                    l.lead_score, l.lead_quality, l.best_time_to_call, l.created_at, l.status, l.company, l.activity_summary, l.note
                FROM public.leads l
                ${whereSql}
                ORDER BY l.created_at DESC
                `,
            ...
            if (!rows.length) {
                return this.sendError(res, {}, "No leads found for the given filters", 400);
            }
            ...
            res.setHeader("Content-Disposition", "attachment; filename=multiple_leads_export.xlsx");
            res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    
            // Send the Excel file as a response
            return res.send(XLSX.write(wb, { bookType: "xlsx", type: "buffer" }));
        } catch (err: any) {
            console.error("exportMultipleLeadsData error:", err);
            return this.sendError(res, err, "Internal server error", 500);
        }
    };
